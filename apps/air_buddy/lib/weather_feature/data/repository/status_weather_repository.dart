...
class StatusWeatherRepository extends IStatusWeatherRepository {
  final HttpService statusHttpService =
      getIt.get<HttpService>(instanceName: 'tmd');
  final HttpService cityHttpService =
      getIt.get<HttpService>(instanceName: 'waqi');

  @override
  Future<List<ForecastModel>> getWeatherForecast(String city) async {
    try {
      final statusWeather = await statusHttpService.get(
          '/api/WeatherForecast7Day/weather-forecast-7day-by-province?Sorting=weatherForecast7Day.recordTime%20asc&FilterText=$city&MaxResultCount=7&culture=th-TH');
      ...
      throw ('Error to call api : $e');
    }
  }

  @override
  Future<CityStationModel> getWeatherByCity(String city) async {
    final cityWeather = await cityHttpService
        .get('/feed/$city/?token=c446149c4593b2dd221b9d37f1b7612a658f4ccc');
